// BgXgid_class.js
// LongGammon Version (28pt Board and 18chequer)
'use strict';
...
class Xgid {
  constructor(xgid) {
    ...
    this._ptno = new Array(30);
    this._ptcol = new Array(30);
    this._pip = [0, 0];
    this._boff = [0, 0];
    this._gamesc = [[], []];
    ...
    this._turnpos = ((p) => (this._turn == 1) ? p : 29 - p);
  ...
  _parse_position(pt) {
    ...
    for (let i=0; i<=29; i++) {
      ...
      if (asc == "-".charCodeAt(0)) {
        ...
        if (i< 8) { this._bgarea[1] += this._ptno[i]; }
        if (i<22) { this._gmarea[1] += this._ptno[i]; }
        this._pip[1] += this._ptno[i] * (29 - i); // ピップ数を計算
      } else if (asc >= "A".charCodeAt(0) && asc <= "Z".charCodeAt(0)) {
        this._ptno[i] = asc - "A".charCodeAt(0) + 1;
        this._ptcol[i] = 1;
        this._boff[0] -= this._ptno[i];
        if (i>21) { this._bgarea[0] += this._ptno[i]; }
        if (i> 7) { this._gmarea[0] += this._ptno[i]; }
...
//public and private(helper) functions
  ...
  moveChequer(pos, move, turn) {
    ...
    for (let mv of BgUtil.cleanupMoveStr(move, this._xgid)) {
      frto = mv.split("/");
      fr = parseInt(frto[0]); fpt = (turn == 1) ? fr : 29 - fr;
      to = parseInt(frto[1]); tpt = (turn == 1) ? to : 29 - to; bar = (turn == 1) ? 0 : 29;
      if (isNaN(fr)) { break; }
  ...
  isValid() {
    if (this._dice_ary[1] > 7 || this._dice_ary[1] < 0) { return false; }
    if (this._dice_ary[2] > 7 || this._dice_ary[2] < 0) { return false; }
