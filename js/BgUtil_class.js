...
class BgUtil {
  ...
  static cleanupMoveStr(movestr, xgid) {
    ...
    let strary = str.replace(/off/gi, "0").replace(/bar/gi, "29").trim().replace(/\s+/, " ").split(" ");
  ...
  static makeMoveStr(ary, n) {
    ...
    for (let i = 0; i < reduced.length; i++) {
      out = reduced[i].replace("/0", "/Off").replace("29/", "Bar/");
      if (i == idxary[n]) {
